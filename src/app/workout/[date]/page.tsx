...
export default function WorkoutPage({
  ...
  const completedExercises = workout.exercises.filter(e => e.sets.every(s => s.completed)).length
    ...
    <>
      ...
      <div className="min-h-screen bg-[var(--background)] pb-20">
        ...
        <header className="bg-[var(--surface)] shadow-sm sticky top-0 z-40">
          <div className="max-w-4xl mx-auto px-4 py-4">
            ...
            <div className="flex justify-between mt-2 text-sm">
              ...
              <span className="font-semibold text-[var(--primary)]">{Math.round(progressPercentage)}%</span>
        ...
        <main className="max-w-4xl mx-auto px-4 py-6">
                          ...
                          <div className="flex items-center gap-3">
                            <div className="flex items-center gap-1">
                              <input
                                type="number"
                                value={set.weight}
                                onChange={(e) => updateWeight(exercise.id, set.id, parseFloat(e.target.value) || 0)}
                      ...
                      >
                        ...
                        <div className="flex gap-2">
                          {[-2.5, 2.5, 5].map(increment => (
                            <button
                              key={increment}
                              onClick={() => updateWeight(exercise.id, set.id, Math.max(0, set.weight + increment))}
                              className="min-w-[44px] px-3 py-2 bg-[var(--surface)] active:bg-[var(--primary)] active:text-white hover:bg-[var(--primary)] hover:text-white border border-[var(--border)] rounded-lg text-sm font-medium transition-colors touch-manipulation"
                              disabled={saving}
                            >
          ...
          <div className="grid grid-cols-2 gap-4 mb-6">
            ...
            >
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-2xl font-bold">{completedExercises}/{workout.exercises.length}</span>
              </div>
          ...
          >
            {completedSets === totalSets ? 'Complete Workout 🎉' : completedSets > 0 ? 'Finish Early' : 'Complete some sets first'}
          </motion.button>
        </main>
